...
class WC_CryptAPI_Gateway extends WC_Payment_Gateway
{
        ...
        if (!empty(WC_CryptAPI_Gateway::$COIN_OPTIONS)) {
            $this->form_fields = array(
                ...
                'show_crypto_logos' => array(
                    'title' => __('Show crypto logos in checkout', 'cryptapi'),
                    'type' => 'checkbox',
                    'label' => sprintf(__('Enable this to show the cryptocurrencies logos in the checkout %1$s %2$s Notice: %3$s It may break in some templates. Use at your own risk.', 'cryptapi'), '<br/>', '<strong>', '</strong>'),
                    'default' => 'no'
                ),
                ...
                'fee_order_percentage' => array(
                    ...
                    'description' => sprintf(__('Set the CryptAPI service fee you want to charge the costumer. %1$s %2$s Note: %3$s Fee you want to charge your costumers (to cover CryptAPI\'s fees fully or partially).', 'cryptapi'), '<br/>', '<strong>', '</strong>')
                ),
                ...
                'api_key' => array(
                    ...
                    'type' => 'text',
                    'default' => '',
                    'description' => sprintf(__('Insert here your BlockBee API Key. You can get one here: %1$s', 'cryptapi'), '<a href="https://dash.blockbee.io/" target="_blank">https://dash.blockbee.io/</a>')
                ),
            );
    ...
    function order_status()
    {
        ...
        try {
            ...
            if (((int)$order->get_meta('cryptapi_last_checked') + 60) < time() && (int)$order->get_meta('cryptapi_cancelled') === 0 && !$order->is_paid()) {
                    ...
                    <div class="ca_payments_wrapper">
                        ...
                        if (intval($this->order_cancelation_timeout) != 0) {
                            ...
                            <span class="ca_notification_cancel" data-text="<?php echo __('Order will be cancelled in less than a minute.', 'cryptapi'); ?>">
                                    <?php echo sprintf(__('This order will be valid for %s', 'cryptapi'), '<strong><span class="ca_cancel_timer" data-timestamp="' . $cancel_timer . '">' . date('H:i', $cancel_timer) . '</span></strong>'); ?>
                                </span>
                            <?php
                        }
                        ?>
                        ...
                        if ($this->show_branding) {
                            ...
                            <div class="ca_branding">
                                <a href="https://cryptapi.io/" target="_blank">
                                    ...
                                    <img width="94" class="img-fluid" src="<?php echo CRYPTAPI_PLUGIN_URL . 'static/files/200_logo_ca.png' ?>" alt="Cryptapi Logo"/>
                                </a>
                            </div>
                            <?php
                        }
    ...
    function ca_cronjob()
    {
        ...
        foreach ($orders as $order) {
            ...
            if ((int)$order->get_meta('cryptapi_cancelled') === 0 && ($order_timestamp + 86400) > time()) {
                $this->validate_logs($order, $history);
            }
    ...
    function add_email_link($order, $sent_to_admin, $plain_text, $email)
    {
        ...
        if ($email->id == 'customer_on_hold_order') {
            WC_CryptAPI_Gateway::$HAS_TRIGGERED = true;
            echo '<a style="display:block;text-align:center;margin: 40px auto; font-size: 16px; font-weight: bold;" href="' . $this->get_return_url($order) . '" target="_blank">' . __('Check your payment status', 'cryptapi') . '</a>';
        }
    }
